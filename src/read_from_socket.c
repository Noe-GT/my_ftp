/*
** EPITECH PROJECT, 2025
** ftp
** File description:
** read_from_socket
*/

#include "../include/ftp.h"

static int check_cmd(server_t *server, char *buffer, int client_i)
{
    char *clean = clean_str(buffer, "\n\r");
    char **tokens = my_str_to_word_array(clean, " ");
    int len = my_array_len(tokens);

    if (len == 1 && strcmp(tokens[0], "NOOP") == 0)
        return noop_cmd(server->client_fds[client_i].fd);
    if (len == 1 && strcmp(tokens[0], "QUIT") == 0)
        return quit_cmd(server, client_i);
    if ((len == 1 || len == 2) && strcmp(tokens[0], "HELP") == 0)
        return help_cmd(server, client_i, tokens, len);
    return 0;
}

int read_from_socket(server_t *server, int client_i)
{
    char buffer[1000];
    int len = read(server->client_fds[client_i].fd, buffer, 1000 - 1);

    buffer[len] = '\0';
    return check_cmd(server, buffer, client_i);
}
